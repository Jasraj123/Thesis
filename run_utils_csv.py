...
def get_estimates(dataset_train, dataset_val, delta, significance_level = 0.05):
    ...
    # Extract all covariates from the dataset
    # Instead of assuming a single 'X' column, check for multiple covariate columns
    if 'X' in dataset_train.columns:
        # Backward compatibility for single covariate case
        X_train = np.array(dataset_train['X']).reshape(-1, 1)
    else:
        # For multiple covariates, collect all covariate columns
    ...
    # Extract validation set covariates similarly
