...
label_dic = {
    ...
    'True ATE': r"$\tau$" + " (Estimated in " + r"$\mathcal{D}^1$" + ")"
}
...
def plot_results(exp_name="csv_data"):
    ...
    for alpha_index in range(len(alpha_list)):
        ...
        read_dir = os.path.join(path, "code", exp_name, "experiments_u", "exp_results", 
                                f"alpha_{alpha_list[alpha_index]}", "unconfounding_0")
        ...
        # Plot the true ATE as a vertical line
        ax[alpha_index].axvline(
            np.mean(np.array(true_ate)), 
            color='r', 
            linestyle='dashed', 
            label=label_dic['True ATE']
        )
        
        # Set y-axis label to show alpha value
        ax[alpha_index].set_ylabel(
            r"$\alpha = $" + str(alpha_list[alpha_index]), 
            fontsize=font_size
        )
    ...
    plt.savefig(f"{path}/figs/results/{exp_name}/experiments_u.png", bbox_inches="tight", dpi=300)
    
    # Also display the figure
    plt.show()

if __name__ == "__main__":
    # Plot results for the CSV-based experiment
    plot_results(exp_name="csv_data") 